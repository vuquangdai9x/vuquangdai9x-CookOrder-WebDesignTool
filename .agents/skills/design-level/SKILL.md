...
- Keep generated amounts within `stackRange` when practical. Use amount 1 for an unavoidable remainder. Never turn exact supply, deadlock safety, or an omitted amount preference into an `amount = 1` requirement; single-unit queues require an explicit designer override. Use `set_queue_slot_amount`, `split_queue_slot`, and `merge_queue_slots` for manual correction.
