...
export interface NodeCustomerSectionDeps {
   ...
   * this section's own state changes, so no re-render happens.
   */
  onHoverCustomer?(index: number | null): void;
}
...
function customerCard(
  ...
  if (cost && estimate) {
    // Red / yellow / green by how much the player has to guess for this
    // customer — see estimateDifficulty.ts's pickQuality.
    const quality = pickQuality(cost);
    const bar = el("div", {
      class: `difficulty-bar quality-${quality}`,
      title: `${pickQualityLabel(cost)}. Peak grid pressure: ${cost.gridOccupied} cell(s)`,
    });
    bar.style.background = pickQualityColor(quality);
    card.append(bar);
  }

  // Hovering a card lights up the queue tiles and the chart points that this
  // customer's picks belong to. Estimate-only; the host ignores it otherwise.
  if (deps.onHoverCustomer) {
    card.addEventListener("pointerenter", () => deps.onHoverCustomer?.(index));
    card.addEventListener("pointerleave", () => deps.onHoverCustomer?.(null));
  }
