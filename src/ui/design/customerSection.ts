...
export interface CustomerSectionDeps {
  ...
  /** Legacy Design only: converts the live customer string to the active graph format. */
    ...
    headerButtons: (sec) => [
      ...(deps.convertToNewFormat
        ? [button("Convert to New Format", () => {
            openStringConversionDialog("Converted Customer String", () =>
              deps.convertToNewFormat!(serializeCustomers(sec.draft)),
            );
          }, { title: "Convert this live legacy customer draft to the active graph's current ids and bracketed dish format" })]
        : []),
