...
import { openAutoGenerateDialog } from "./autoGenerateDialog.ts";
...
export function createCustomerSection(deps: CustomerSectionDeps): Section<CustomerConfig[]> {
  const section: Section<CustomerConfig[]> = new Section<CustomerConfig[]>({
    ...
    headerButtons: (sec) => [
      ...
      }, { class: "small-btn", title: "Generate a customer sequence from a dish-count list, allowed ingredients, and a complexity curve" }),
    ],
    writeToSheet: {
      write: (draft) =>
        writeRowToSheet(
