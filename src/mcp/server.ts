...
register("add_queue_ingredient", "Add explicitly selected pickup copies to one lane; this never generates a queue.", { ...sessionRevision, lane_id: z.string(), position: z.number().int().nonnegative(), ingredient: z.string(), count: z.number().int().min(1).max(100).optional(), provisional: z.boolean().optional() }, (a) => service.addQueueIngredient(String(a.session_id), Number(a.expected_revision), { laneId: String(a.lane_id), position: Number(a.position), ingredient: String(a.ingredient), count: a.count as number | undefined, provisional: a.provisional as boolean | undefined }));
