...
register("add_queue_ingredient", "Add explicitly selected pickup slots to one lane; this never generates a queue. `count` = slots added, `amount` = pieces per slot (a bag of 2+ lands on one grid cell and drains a piece at a time).", { ...sessionRevision, lane_id: z.string(), position: z.number().int().nonnegative(), ingredient: z.string(), count: z.number().int().min(1).max(100).optional(), amount: z.number().int().min(1).max(100).optional(), provisional: z.boolean().optional() }, (a) => service.addQueueIngredient(String(a.session_id), Number(a.expected_revision), { laneId: String(a.lane_id), position: Number(a.position), ingredient: String(a.ingredient), count: a.count as number | undefined, amount: a.amount as number | undefined, provisional: a.provisional as boolean | undefined }));
register("set_queue_slot_amount", "Set how many pieces one queue slot holds (a bag). 1 makes it a plain slot again.", { ...sessionRevision, slot_id: z.string(), amount: z.number().int().min(1).max(100) }, (a) => service.setQueueSlotAmount(String(a.session_id), Number(a.expected_revision), String(a.slot_id), Number(a.amount)));
