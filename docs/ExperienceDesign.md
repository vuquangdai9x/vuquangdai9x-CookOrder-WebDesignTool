...
| 2.3a | Chained recipe — mid-hop | A multi-step recipe's tool finishes but has more tools to visit (`chainTools`) → `tool-to-tool` flight launches instead of producing output (e.g. Potato: Cutting Board → Fryer) | Prototype (flight only; same generic fly clone as 2.3, no dedicated flourish) | Item's icon clone flies from the finishing tool's slot to the next tool's slot | Diegetic-kitchen: a shorter, lower-pitched variant of 2.3's ding — reads as "moved on", not "done" | Light Impact |
| 2.3b | Chained recipe — next tool full | The chain's next tool has no free slot | Prototype (no cue — the item just silently stays put and retries every tick) | *Proposed:* the occupied slot the item is waiting to enter could pulse briefly to hint "something's queued for this" | UI: none today | None |
| 2.4 | Unpacked amount cannot fully land | `packingMode = "unpacked-raw"` and the tool plus grid lack capacity for every item | Prototype (same disabled-tile treatment as 1.5) | Front tile disabled + tooltip | UI: same "no" cue as 1.5 | Selection |
| 2.5 | Raw parked on grid | Its tool is full, or Wait-order has no matching active order — raw lands on the grid | Prototype | Dimmed/grayscale icon on the grid cell + "waiting" badge | Diegetic-kitchen: a soft *set-down* clink (distinct from 3.1's landing sound — this one reads as "put aside", not "delivered") | Light Impact |
...
| 4.1a | Ingredient serves a customer directly, skipping the grid | A freshly cooked item or a no-tool-needed queue pick finds a customer already waiting for it → `tool-to-customer` / `queue-to-customer` flight launches straight from the tool/queue instead of landing on the grid first | Prototype (flight only; lands the same as 4.1) | Item's icon clone flies straight from the tool slot / queue tile to the dish chip — never touches the grid | Same as 4.1, with a faster travel arc since it's covering more distance in one hop; no separate landing sound (4.1's plays on arrival) | Light Impact |
| 4.1b | Multi-use ingredient served, uses remain | A `grid-to-customer` flight lands on a grid cell whose ingredient has `usesLeft > 1` (`consumeCookedCell`) | Prototype (badge updates only, no dedicated flourish — same treatment as 3.4's stack-count badge) | The cell's top-right `×N` uses-left badge decrements instead of the cell clearing | UI: none today | None |
