...
- The shared Play/estimator **Packing mode behavior** controls queue amounts. **Packing raw** keeps an ordinary amount in one draining bag and a `multipleUsage` amount in one reusable object. **Unpacked raw** is the default and expands the amount into separate one-use items: at most one enters a free tool immediately, each remainder needs its own grid cell, and the whole pick is blocked unless every item has a landing.
- The shared Play/estimator **Tool process behavior** controls demand gating. **Auto** lets graph-auto recipes run whenever possible while manual recipes still wait for demand. **Wait-order** makes every recipe wait until an active customer order needs its output path; unmatched singles park on the grid and unmatched packed amounts stay in their bag. Estimator runs and replays retain the selected pair.
...
- There are **1–2 concurrent serveable slots** (selected by the dynamic serve window). The next **three** customers are previewed beside them. A preview reveals only each ordered composite's identity/emoji (burger, coffee, cupcake, …), never its selected ingredient combination; pending customers still cannot be served until a slot frees up.
...
- **Difficulty estimate**: the next-three-composite preview contributes only low-confidence expected demand. The configurable pick interval (default 1 gameplay second) remains the minimum delay between decisions. The authored attempt may overlap work; retries use the same interval and additionally settle progressable tool/merge work before the next decision. Before a pick, the estimator projects occupied grid cells plus committed outputs and the candidate footprint. If a run cannot win, it accumulates visibility-safe failure pressures (grid, dirty, urgency, scarcity, chain stall, guessing, and pick pacing), applies those lessons over the next strategic preset, and retries up to the configured 0–10 times. The final retry can use a bounded beam of previously failed legal routes, retaining only the 30 partial runs with the most served customers, least unfinished active demand, and lowest grid occupancy. Candidate scoring still uses only active orders, the three preview cards, and visible queue rows; no queue/customer identity is persisted as failure knowledge. Grid/dirty failure grows cadence multiplicatively and tightens the safety reserve. Customer patience expiry is warning-only for estimation: the logical solve continues and the final result lists the exact one-based customer numbers that timed out. Save Me is never invoked by the solver and remains a separate economy estimate. The solver stops at the first win or reports the best failed run.
