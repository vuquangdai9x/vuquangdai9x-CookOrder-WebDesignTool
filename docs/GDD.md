...
- The shared Play/estimator **Packing mode behavior** controls queue amounts. **Packing raw** keeps an ordinary amount in one draining bag and a `multipleUsage` amount in one reusable object. **Unpacked raw** expands the amount into separate one-use items: at most one enters a free tool immediately, each remainder needs its own grid cell, and the whole pick is blocked unless every item has a landing.
