...
- **Difficulty estimate**: the next-three-composite preview contributes only low-confidence expected demand. If the first run cannot win, the estimator retries up to the configured 0–10 times with strategic scoring presets, then randomized scoring after all presets are used, stopping at the first win or reporting the best failed run.
