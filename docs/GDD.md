...
- **Difficulty estimate**: the next-three-composite preview contributes only low-confidence expected demand. If a run cannot win, the estimator accumulates visibility-safe failure pressures (grid, dirty, urgency, scarcity, chain stall, and guessing), applies those lessons over the next strategic preset, and retries up to the configured 0–10 times. Randomized presets follow the fixed presets; the solver stops at the first win or reports the best failed run. Learning never stores queue, ingredient, or customer identities.
