...
- **Difficulty estimate**: the next-three-composite preview contributes only low-confidence expected demand. The configurable pick interval (default 1 gameplay second) is the minimum delay between decisions, matching an autoplay bot that can act while animations continue. Before a pick, the estimator projects occupied grid cells plus committed outputs and the candidate footprint; it waits for only the next useful completion when that load cannot fit. If a run cannot win, it accumulates visibility-safe failure pressures (grid, dirty, urgency, scarcity, chain stall, guessing, and pick pacing), applies those lessons over the next strategic preset, and retries up to the configured 0–10 times. Grid/dirty failure grows cadence multiplicatively and tightens the safety reserve. Customer patience expiry is warning-only for estimation: the logical solve continues and the final result lists the exact one-based customer numbers that timed out. Randomized presets follow the fixed presets; the solver stops at the first win or reports the best failed run. Learning never stores queue, ingredient, or customer identities.
